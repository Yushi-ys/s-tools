...
import { CopyOutlined, FileImageOutlined, FileTextOutlined, HomeOutlined } from "@ant-design/icons";
...
const RenderItem = ({ item, index, updateSelectedIndex, handleCopy, selectIndex }: IRenderItemProps) => {
    const { type, data, width, height } = item;

    const renderContent = useMemo(() => {
        if (type === 'text') return <div>{data}</div>
        if (type === 'image') return <Flex align="flex-end" justify="space-between" style={{ paddingRight: '0.5rem' }}><Image width={100} height={100} src={data} /><div>{width} * {height}</div></Flex>
        return <></>
    }, [type, data])
    ...
    return <div className={
        ...
        <div className={styles.timestamp}>
            ...
            <div className={styles.icon} onClick={(e) => handleCopy(e, item)}><CopyOutlined /></div>
        </div>
    </div>
}
...
const Clipboard: React.FC = () => {
    ...
    const handleCopy = useMemoizedFn(async (e: React.MouseEvent, item: IClipboardItem) => {
        e.stopPropagation();
        ...
        if (success) {
            messageApi.open({
                type: 'success',
                content: item.type === 'image' ? '图片已复制到剪贴板' : '文本已复制到剪贴板',
            });
        } else {
            messageApi.open({
                type: 'error',
    ...
    return <>
        <div className={styles.tabs}>
            <Tabs defaultActiveKey={COPYKEYBOARDTYPE.ALL} activeKey={selectTab} items={items} onChange={handleChangeTab} />
        </div>
        <div className={styles.dataSourceWrapper}>
            {contextHolder}{
                isLoading ? '加载中..........' : dataSource.map((item, index) => <RenderItem item={item} index={index} selectIndex={selectIndex} updateSelectedIndex={updateSelectedIndex} handleCopy={handleCopy} key={'clipBorad' + index} />)
            }
        </div>
    </>
}
